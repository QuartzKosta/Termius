...
/** POST /api/auth/login
 *  Body: { warden_name: string, password: string }
 *  On success, sets an HTTP-only cookie and returns { ok, player }.
 */
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: player, error } = await supabase
      .from("players")
      .select("id, warden_name, password_hash")
      .eq("warden_name", warden_name)
      .single();
    ...
    const res = NextResponse.json({
      ok: true,
      player: { id: player.id, warden_name: player.warden_name },
    });
    res.cookies.set(PLAYER_COOKIE, player.id, {
      httpOnly: true,
      sameSite: "strict",
      path: "/",
...
/** GET /api/auth/login — alias of /session (checks current cookie). */
